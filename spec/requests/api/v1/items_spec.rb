...
RSpec.describe "Items", type: :request do
  describe "获取账目" do
    ...
    it "按时间筛选" do
      user1 = create :user
      item1 = create :item, amount: 100, happen_at: "2018-01-02", user: user1
      item2 = create :item, amount: 100, happen_at: "2018-01-02", user: user1
      item3 = create :item, amount: 100, happen_at: "2019-01-01", user: user1
    ...
    it "按时间筛选（边界条件）" do
      item1 = create :item, amount: 100, happen_at: "2018-01-01"

      get "/api/v1/items?happen_after=2018-01-01&happen_before=2018-01-02", headers: item1.user.generate_auth_header
      expect(response).to have_http_status 200
      json = JSON.parse response.body
      expect(json["resources"].size).to eq 1
      expect(json["resources"][0]["id"]).to eq item1.id
    ...
    it "按时间筛选（边界条件3）" do
      user1 = create :user
      item1 = create :item, amount: 100, happen_at: "2018-01-01", user: user1
      item2 = create :item, amount: 100, happen_at: "2019-01-01", user: user1
