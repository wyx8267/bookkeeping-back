...
resource "账目" do
  ...
  get "/api/v1/items" do
    ...
    example "获取账目" do
      ...
      create_list :item, Item.default_per_page+1, amount: 100, happen_at: "2022-10-23", user_id: current_user.id, tag_ids: [tag.id]
      do_request
  ...
  get "/api/v1/items/summary" do
    ...
    parameter :kind, "分组依据", enum: ["happen_at", "tag_id"], required: true
    response_field :groups, '分组信息'
    response_field :amount, '总金额（单位：分）'
